...
function save_tag(tag_id,token) { 
  ...
  //var datas = "&task=tags.edit_tag&name="+tagname+"&"+"slug="+slug+"&"+"id="+tag_id+"&"+token+"=1";

var datas = {};

datas['task']="tags.edit_tag";
datas['name']=tagname;
datas['slug']=slug;
datas['id']=tag_id;
datas[token]='1';
 ...
 jQuery.ajax({ 
  ...
  td_name='<a class="pointer" id="name'+tag_id+'" onclick="edit_tag('+tag_id+',\''+token+'\')"  title="Edit">'+array[0]+'</a>';
  ...
  td_slug='<a class="pointer" id="slug'+tag_id+'" onclick="edit_tag('+tag_id+',\''+token+'\')"  title="Edit">'+array[1]+'</a>';
  ...
  td_count='<a class="pointer" id="count'+tag_id+'" onclick="edit_tag('+tag_id+',\''+token+'\')"  title="Edit">'+array[2]+'</a>';

  jQuery("#td_count_" + tag_id).html(td_count);

  

  td_edit='<a class="pointer" onclick="edit_tag('+tag_id+',\''+token+'\')" >Edit</a>';
...
function spider_ajax_save(form_id, tr_group,token) {
  ...
  jQuery.post(
    ...
    if (tr_count > bwg_save_count * tr_group) {
      spider_ajax_save(form_id, ++tr_group,token);
      return;
    }
    else {
...
function spider_show_hide_weights() {
  ...
  if (jQuery("#show_hide_weights").val() == 'Show order column') {
    ...
    var page_number;
	page_num=parseInt(jQuery('.paging-input').find('.total-pages').html());
...
function bwg_change_theme_type(type) {
  ...
  jQuery("#type_Mosaic").attr("style", "background-color: #F4F4F4;");
  ...
  jQuery("#type_Masonry_album").attr("style", "background-color: #F4F4F4;");
...
function bwg_check_checkboxes() { 
  ...
  for (var i in ids_array) {
    ...
    if (ids_array.hasOwnProperty(i) && ids_array[i]) {
      ...
      if (jQuery("#check_" + ids_array[i])[0].checked == true) {
  ...
  if(flag) {
    ...
    if(jQuery(".buttons_div_right").find("a").hasClass( "modal" )) {       
      ...
      return true; 
	  jQuery('#no_selection').remove()	 
jQuery('#sbox-content').css('display','');
...
jQuery('#sbox-content').css('display','');
    ...
    jQuery('#draganddrop').show();
jQuery('#no_selection').remove();	
jQuery("#sbox-window").append("<div id='no_selection' style='text-align: center;'><strong style='font-size:22px'>You must select at least one item.</strong></div>");
jQuery('#sbox-content').css('display','none');
